...
const MASTER_KEY =
  '$2a$10$jVbWYJfM5JDHuWwow8Rpve8NXUY.hbdTTHXc6R8XY1KL/9uGI1QdG';
const BIN_ID = '69fa8d52856a682189af20b0';
...
const defaultData = () => ({
  schedule: DAYS.reduce(
    (a, d) => ({ ...a, [d]: { ida: null, volta: null } }),
    {}
  ),
  passengers: DAYS.reduce(
    (a, d) => ({ ...a, [d]: { ida: [], volta: [] } }),
    {}
  ),
});
...
export default function CaronaApp() {
  ...
  const addTrip = () => {
    ...
    const newData = {
      ...
      schedule: {
        ...data.schedule,
        [day]: { ...data.schedule[day], [type]: time },
      },
    };
    update(newData);
  ...
  const removeTrip = (day, type) => {
    const newData = {
      ...
      schedule: {
        ...data.schedule,
        [day]: { ...data.schedule[day], [type]: null },
      },
      passengers: {
        ...data.passengers,
        [day]: { ...data.passengers[day], [type]: [] },
      },
    };
    update(newData);
  ...
  const addPassenger = (day, type) => {
    ...
    const newData = {
      ...
      passengers: {
        ...data.passengers,
        [day]: { ...data.passengers[day], [type]: [...list, name] },
      },
    };
    update(newData);
  ...
  const removePassenger = (day, type, name) => {
    const newData = {
      ...
      passengers: {
        ...
        [day]: {
          ...data.passengers[day],
          [type]: data.passengers[day][type].filter((n) => n !== name),
        },
      },
    };
    update(newData);
  ...
  if (!data) {
    return (
      <div style={s.loading}>
        <div style={s.spinner} />
        <span style={{ color: '#f97316', fontFamily: FONT_SYNE }}>
          Carregando...
        </span>
      </div>
    );
    ...
    <div style={s.root}>
      {toast && (
        <div
          style={{
            ...s.toast,
            background: toast.type === 'warn' ? '#ef4444' : '#22c55e',
          }}
        >
          {toast.msg}
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={s.tabs}>
              ...
              <button
                ...
                style={{ ...s.tab, ...(view === 'admin' ? s.tabActive : {}) }}
              >
                Gerenciar
              </button>
            </div>
      ...
      <main style={s.main}>
        {view === 'admin' && (
          <div style={s.card}>
            ...
            <p style={s.hint}>
              Configure os dias e horarios que vai dar carona.
            </p>
            <div style={s.formRow}>
              <div style={s.formGroup}>
                ...
                <select
                  style={s.select}
                  value={adminInput.day}
                  onChange={(e) =>
                    setAdminInput((p) => ({ ...p, day: e.target.value }))
                  }
                >
                  {DAYS.map((d) => (
              ...
              <div style={s.formGroup}>
                ...
                <select
                  style={s.select}
                  value={adminInput.type}
                  onChange={(e) =>
                    setAdminInput((p) => ({ ...p, type: e.target.value }))
                  }
                >
              ...
              <div style={s.formGroup}>
                ...
                <input
                  type="time"
                  style={s.select}
                  value={adminInput.time}
                  onChange={(e) =>
                    setAdminInput((p) => ({ ...p, time: e.target.value }))
                  }
                />
              </div>
              <button style={s.btnPrimary} onClick={addTrip}>
                + Adicionar
              </button>
            </div>
        ...
        {view === 'board' && (
          <div>
            <p style={s.boardHint}>
              Veja as vagas e adicione seu nome para reservar!
            </p>
                ...
                return (
                  <div key={day} style={s.dayCard}>
                    ...
                    {['ida', 'volta'].map((type) => {
                      ...
                      const routeText =
                        type === 'ida'
                          ? 'Guaruja para Jabaquara'
                          : 'Jabaquara para Guaruja';
                      ...
                      return (
                        <div key={type} style={s.tripBlock}>
                          ...
                          <div style={s.seats}>
                            {seats > 0 ? (
                              <span style={s.seatsOk}>
                                {seats} vaga{seats > 1 ? 's' : ''} disponivel
                              </span>
                            ) : (
                          ...
                          {seats > 0 && (
                            <div style={s.addRow}>
                              <input
                                ...
                                onKeyDown={(e) =>
                                  e.key === 'Enter' && addPassenger(day, type)
                                }
                              />
                              <button
                                style={s.btnJoin}
                                onClick={() => addPassenger(day, type)}
                              >
                                Entrar
                              </button>
                            </div>
              ...
              {!hasAny && (
                <div style={s.empty}>
                  ...
                  <p style={{ fontSize: 14, color: '#9ca3af' }}>
                    O dono do carro precisa adicionar os horarios em Gerenciar.
                  </p>
                </div>
              )}
            </div>
